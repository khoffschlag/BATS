...
function checkExercise(topic, task, userAnswer, targetAnswer, currentTry) {
    ...
    if (currentTry == 2) {
        if (topic == 'decimalConversion') {
            return { result: false, feedback: checkDecimalConversionExercise(userAnswer, targetAnswer).feedback };
        }
        else {
            return { result: false, feedback: bitComparison(userAnswer, targetAnswer).feedback };
        }
    }
